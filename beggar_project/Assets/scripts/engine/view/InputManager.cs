...
namespace HeartUnity.View
{
    ...
    public class InputManager
    {
        ...
        {
            InputDevice device = latestInputDevice.HasValue ? latestInputDevice.Value : InputDevice.KEYBOARD;
            foreach (var b in bb)
            {
                ...
                else
                {
                    ...
                    {
                        // return keyboard binding even if on mouse
                        if (device != InputDevice.CONTROLLER)
                        {
        ...
        public InputDevice? latestInputDevice = null;
        private static InputDevice? PreviousSceneLatestInputDevice = null;
        ...
        {
            PreviousSceneLatestInputDevice = latestInputDevice;
        }

        public void ApplyPreviousSceneLatestDevice()
        {
            if (!PreviousSceneLatestInputDevice.HasValue) return;
            latestInputDevice = PreviousSceneLatestInputDevice;
            PreviousSceneLatestInputDevice = null;
        }
