...
namespace BreakInfinity
{
    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        internal BigDouble LimitDigit(int nDigits)
        {
            if(exponent > nDigits)
                mantissa =  Math.Round(mantissa, nDigits - 1);
            return this;
        }
        ...
        public void ChangeMantissaForStringInteger() {
            var digitNumber = this.Exponent % 3;
            double power = PowersOf10.Lookup(digitNumber);
            var number = (int)(this.mantissa * power);
            this.mantissa = number / power;
        }

        public string ToStringInteger()
        {
            var value = this;
            var digitNumber = value.Exponent % 3;
            //if (digitNumber == 0 && value.Exponent > 0) digitNumber = 3;
            //if (value.Exponent == 0) digitNumber = 1;

            var number = (int)(value.mantissa * PowersOf10.Lookup(digitNumber));
            if (value.TryGetRepresentation(out var rep))
            {
                return number + rep;
            }
            else {
        ...
        internal BigDouble SimpleStringVerssability()
        {
            if (exponent > 3)
        ...
        public string ToStringSimple()
        {
            ...
            double number = Math.Round(value.mantissa * PowersOf10.Lookup(digitNumber), 2);
            //var number = (int)(value.mantissa * PowersOf10.Lookup(digitNumber));
            if (value.TryGetRepresentation(out var rep))
            {
                return number + rep;
    ...
    public static class BigDoubleExtensions
    {
        ...
        public static bool TryGetRepresentation(this BigDouble value, out string representation)
        {
            ...
            // Define the ranges for the exponents and their corresponding representations
            int[] ranges = { 3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36, 39, 42, 45, 48, 51, 54, 57, 60, 63, 66, 69, 72, 75, 78, 81, 84, 87, 90, 93, 96, 300 };
            string[] representations = { "k", "M", "B", "T", "qd", "qn", "sx", "sp", "oc", "nn", "dc", "ud", "dd", "td", "q4", "q5", "s6", "s7", "o8", "n9", "vg", "uv", "dv", "tv", "qv", "qn", "sx", "sv", "og", "nv", "tr", "ut", "un" };

            for (int i = 0; i < ranges.Length; i++)
            {
                if (exponent - 3 < ranges[i])
                {
