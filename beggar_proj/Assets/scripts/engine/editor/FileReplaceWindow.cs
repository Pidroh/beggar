...
public class FileReplaceWindow : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        Dictionary<string, List<FileReplaceConfiguration.Entry>> tagToEntries = ReadTags();
        this.tagToEntries = tagToEntries;
    }

    private static Dictionary<string, List<FileReplaceConfiguration.Entry>> ReadTags()
    {
        string[] configurationPaths = AssetDatabase.FindAssets("t:FileReplaceConfiguration");
        var tagToEntries = new Dictionary<string, List<FileReplaceConfiguration.Entry>>();
    ...
    private void OnGUI()
    {
        ...
        foreach (var tag in tagToEntries.Keys)
        {
            ...
            {
                ReplaceFilesForTag(tag, tagToEntries);
            }
        }
    }

    public static bool ReplaceFilesForTag(string tag) 
    {
        return ReplaceFilesForTag(tag, ReadTags());
    }

    private static bool ReplaceFilesForTag(string tag, Dictionary<string, List<FileReplaceConfiguration.Entry>> tagToEntries)
    {
        var success = false;
    ...
    private static void EnsureMainObjectNameMatchesFilename(string assetFilePath)
    {
